...
wez.on("format-tab-title", function(tab, _, _, conf, _, _)
  ...
  return {
    { Background = { Color = bg } },
    { Foreground = { Color = fg } },
    { Text = utilities._space(title, 1, 1) },
  }
end)
  ...
  if options.modules.pane.enabled then
    ...
    for _, p in ipairs(panes_with_info) do
      if p.is_active and p.is_zoomed then
        table.insert(left_cells, { Foreground = { Color = "#F9E2AF" } })
        table.insert(
          left_cells,
          { Text = "zoom" .. utilities._space(wez.nerdfonts.md_fullscreen, options.separator.space) }
        )
      end
    end
  end
